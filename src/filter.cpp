...
void LMeanShiftFilter2D<T>::Filter(LImage<T> &imageFrom, int bandFrom, LImage<T> &imageTo, int bandTo, int sampleSize)
{
	...
	//子图像大小
	int subWidth = (width + sampleSize - 1) / sampleSize;		//此操作是为了保证子图像大小容纳源图像
	...
	for (j = 0; j < height; j++)
		for (i = 0; i < width; i++, xyData += imageBands + 2, fromData += imageBands)	//各指针自增
		{
			//有两个通道是存放i * invXY和j * invXY的，即空间信息
			xyData[0] = (T)(i * invXY), xyData[1] = (T)(j * invXY);
			for (k = 0; k < imageBands; k++)		//逐通道
				xyData[k + 2] = (T)(fromData[k] * invLuv);	//剩下的通道存放颜色信息
		}

	//最大最小阈值
	double rangeMins[3], rangeMaxs[3];
	xyData = &xyImage(0, 0, 2);	//？？没太懂
	...
	int *buckets, *bucketList, bucketNeighs[27], noBuckets[3];		//27个距离中心吧
