...
//一元势函数，即基于像素的数据项的训练
void LDenseUnaryPixelPotential::Train(LList<char *> &trainImageFiles)		//输入为待训练的文件
{
	int i;
...
void LDenseUnaryPixelPotential::CalculateIntegralImages(LLabImage &labImage, LIntegralImage ***integralImage, int subSample, int *width, int *height, char *imageFileName)
{
	...
	*height = labImage.GetHeight();
	...
	//特征总数特征数量*每个特征的聚类中心个数
	totalFeatures = 0;
	for (i = 0; i < features.GetCount(); i++) totalFeatures += buckets[i];

	//->feature->texton->map的三维数组
	//创建特征总数个整数图像，每个特征有n个距离中心，每一个中心算作是一个texton，一个texton产生一个整数图
	*integralImage = new LIntegralImage *[totalFeatures];	//每个指针代表一个整数图
	...
	for (i = 0; i < features.GetCount(); i++)
	{
		...
		//如没有训练好，就重新提取特征，即分配到不同聚类中心
		else
			((LDenseFeature *)features[i])->Discretize(labImage, dataImage, imageFileName);

		//通道数
		int bands = dataImage.GetBands();		//好像是1
		for (j = 0; j < buckets[i]; j++, offset++)		//每个特征的距离中心数量
		{
			int count = 0;

			unsigned short *dataFrom = dataImage.GetData();		//指向特征数据
			//统计每个中心的元素个数
			for (k = 0; k < *height; k++)
			...
			if (!count)
				(*integralImage)[offset] = NULL;		//如果为空
			//如果不为空，生成整数图
			else
			{
				//如果小于16个，生成半字节整数图
				if (count < 16) (*integralImage)[offset] = new LIntegralImageHB();		//半字节
				//若果小于16个，生成1字节整数图
				else if (count < 256) (*integralImage)[offset] = new LIntegralImage1B();		//1字节
				//如果小于65536个，生成2字节整数图
				else if (count < 65536) (*integralImage)[offset] = new LIntegralImage2B();		//2字节
				//如果特别大，生成4字节整数图
				else (*integralImage)[offset] = new LIntegralImage4B();			//4字节
...
void LDenseUnaryPixelPotential::InitTrainData(LList<char *> &trainImageFiles)
{
	...
	//申请内存，申请大小为特征维数的int内存块
	buckets = new int[features.GetCount()];
	//
	for (i = 0; i < features.GetCount(); i++)
		buckets[i] = features[i]->GetBuckets();	//聚类中心个数

	//整数图
	integralImages = new LIntegralImage **[fileSum];	//三维数组
	validPointsX = new int *[fileSum];		//二维数组
	validPointsY = new int *[fileSum];		//二维数组
	pointsSum = new int[fileSum];		//一维数组
	...
	if (domain == costDomain) for (i = 0; i < classNo; i++)
	{
		...
		else if ((i != label) && (!layer->active[i]))
		{
			...
			for (j = 0; j < nodeCount; j++)
				if (layer->parent->labels[j] == i)
					g->add_edge(nodes[layer->nodeOffset + i], nodes[layer->parent->nodeOffset + j + layer->parent->BinaryNodes() - 1], 0, cost);
		}
	}
}
...
void LCooccurencePairwiseImagePotential::Train(LList<char *> &trainImageFiles)
{
	...
	for (i = 0; i < trainImageFiles.GetCount(); i++)
	{
		...
		for (k = 0; k < classNo; k++)
			for (j = 0; j < classNo; j++)
				if ((thisOccurence[k]) && (thisOccurence[j])) classCooccurence[k * classNo + j]++;
	}
...
void LCooccurencePairwiseImagePotential::Initialize(LLabImage &labImage, char *imageFileName)
{
	...
	{
		double p1 = classOccurence[i] / (double)total;		//目标1出现的概率
		double p2 = classOccurence[j] / (double)total;		//目标2出现的概率
		double p12 = classCooccurence[i * classNo + j] / (double)total;		//目标1和2共同出现的概率
		p1 = max(p1, 1e-10),p2=max(p2,1e-10);		//防止除0
		printf("p1=%lf	p2=%lf	p12=%lf\n", p1, p2, p12);
		//共生概率计算公式
		double p = 1 - (1 - p12 / p2) * (1 - p12 / p1);
		printf("p=%lf\n", p);
		if (p > 1) p = 1;		//p最大值
		if (p < 1e-6) p = 1e-6;		//p最小值
		printf("p=%lf\n", p);
		//转换为共生代价
		pairwiseCosts[i * classNo + j] = -pairwiseImageFactor * points * log(p);	//乘以权重因子取-log
		printf("pairwise[%d] = %lf\n", i * classNo + j, pairwiseCosts[i * classNo + j]);
	}
}
